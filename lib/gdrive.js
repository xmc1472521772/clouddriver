...
async function downloadFile(fileId, res, fileName) {
  ...
  // 并行获取文件元数据和文件内容流，避免串行等待，减少首字节延迟
  const metaPromise = drive.files.get({ fileId, fields: 'name,mimeType,size' });
  const streamPromise = drive.files.get(
    { fileId, alt: 'media' },
    { responseType: 'stream' }
  );

  let fileMeta, response;
  try {
    [fileMeta, response] = await Promise.all([metaPromise, streamPromise]);
  } catch (err) {
    // 若其中一个请求失败，清理可能已创建的流以避免资源泄漏
    streamPromise
      .then((r) => { if (r?.data?.destroy) r.data.destroy(); })
      .catch(() => {});
    throw err;
  }
